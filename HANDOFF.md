...
`skilr` is our local, native, zero-token implementation of that pattern.
...
line: "You have a `skilr` CLI. Use it."

When the agent needs a skill:
1. Runs `skilr query "task description"` → gets top-K matches (name + score)
2. Runs `skilr load <name>` → full SKILL.md prints to stdout → enters context
...
- Read AND write. `skilr add` ingests new skills immediately.
...
Scanned skill sources (configurable):
  ...
  ~/.config/skilr/skills/**/SKILL.md   (user-local)
```
...
```
You have a local skills router: `skilr`.
- `skilr list` — see what skills exist (name + 1-line description)
- `skilr query "<task>"` — semantic search, returns top matches
- `skilr load <name>` — inject the full skill instructions into context
Use it proactively when a task looks like it matches a known workflow.
Never guess skill contents — always `skilr load` before following a skill.
```

Slash-command shortcut:
- User types `/deploy` or `/skill deploy` → agent runs `skilr load deploy` directly.
- User describes a task → agent runs `skilr query` first, then `skilr load` on the top hit.
...
- Entry point: `skilr = skilr.cli:main`
- `pipx install -e .` for local install

### Phase 1 — DB + sync (1-2h)
- SQLite schema above, FTS5 enabled
- `skilr sync` walks configured dirs, parses YAML frontmatter from SKILL.md,
  computes sha256, embeds, upserts
- Handles deletes (skills in DB not on disk → prune)
- `skilr list` works

### Phase 2 — Query + load (1h)
- `skilr query` hybrid search (FTS5 top 20 → cosine rerank → top K)
- `skilr load` prints full file, bumps use_count
- `--json` output mode

### Phase 3 — Watch + polish (1h)
- `skilr add <path>` for one-shot ingestion
- `skilr doctor`, `skilr status`
- Config file at `~/.config/skilr/config.toml` with skill dir list

### Phase 4 — Agent wiring (30 min)
- Replace `<available_skills>` injection in Hermes with the 50-token stub
- Add a test prompt: "deploy the frontend" → verify agent calls `skilr query`
  then `skilr load` instead of guessing
...
```toml
# ~/.config/skilr/config.toml
[general]
db_path      = "~/.config/skilr/skills.db"
model        = "BAAI/bge-small-en-v1.5"
top_k        = 3
cache_model  = "~/.config/skilr/model"
...
- No skill EXECUTION — skilr only stores, finds, and serves skill text. The
...
- `skilr query` returns correct skill in top 3 for ≥90% of realistic task
  phrasings across a 50-query eval set.
- `skilr sync` completes in <5s for 100 skills on M-series CPU.
- Adding a new skill file and running `skilr sync` makes it queryable
