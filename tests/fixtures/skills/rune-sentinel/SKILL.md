---
name: rune-sentinel
description: Rune sentinel workflow (compiled from Rune kit)
tags: [rune, sentinel]
---
...
If `.rune/` directory exists, invoke `the rune-integrity-check rule file` (L3) on all `.rune/*.md` files and any state files in the commit diff.
